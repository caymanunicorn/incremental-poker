...
$("#play-button").mousedown(function() {
    ...
    playAHand();
    if (gameProgressCount === 5) {  // change how many play throughs are required before the first upgrade is visible
        $("#upgrade-heading").css("visibility", "visible");
        $("#upgrade-1").css("visibility", "visible");
    }  
...
$("#higher-stakes").mousedown(function() {
    if(stakes < 99000000) {
        if(stakes % 9 === 4) {  // this will always return a remainder of 4 if the stake is a $400, $4000, etc. Then we multiply the stake by 2.5x to get to $1000 etc
            stakes = stakes * 2.5;  
        }else {
...
$("#lower-stakes").mousedown(function() {
    ...
    if(stakes > 19) {
        if(stakes % 9 === 1) {  // this will always return a remainder of 1 if the stake is a $100, $1000, etc. Then we divide the stake by 2.5x to get to $400 etc
            stakes = stakes / 2.5;
        }else {
        stakes = stakes / 2;
        }
...
function playAHand() {
    ...
    if (gameProgressCount > 20) {                       // change how many play throughs are required before the upgrade is visible
        $("#higher-stakes").css("visibility", "visible");
    }  
    ...
    if (gameProgressCount > 35) {                       // change how many play throughs are required before the upgrade is visible
        $("#upgrade-2").css("visibility", "visible");
        $("#upgrade-3").css("visibility", "visible");
    }  

    if (gameProgressCount > 100) {                       // change how many play throughs are required before the upgrade is visible
        $("#upgrade-4").css("visibility", "visible");
    }  
...
function dealCards() {
    ...
    $oneCard.animate({          // animate this card to fall down into its dealt position
        top: cardDealtPosition[dealCardsCount]
    }, dealSpeed, dealCards // a recursive function using the callback function which continues until dealCardsCount exceeds the number of cards
    )
    dealCardsCount++;
...
function call(){
    ...
    $("#their-bet").css("visibility", "visible").text("$" + betSize.toLocaleString("en")).animate({
        ...
        $("#their-bet").delay(700).animate({ // animate their bet to move up and disappear
        top: "30%",
        left: "30%",
        opacity: "0"
        }, function(){
...
function lossAnimation(){
    ...
    $("#win-or-loss").text("LOSS").css("visibility", "visible").fadeOut(0).delay(700).fadeIn(loseHeadAnimation).delay(700).fadeOut(function(){
...
function winHeadAnimation(){  // recursive function to make a bobblehead
    if (headBobCount < 14){
        $("#male-face").animate({
            top: "61.5%"
        }, 100, function(){
...
function loseHeadAnimation(){ // recursive function to make a bobblehead
    if (headBobCount < 14){
        $(opponentsArray[opponent]).animate({
            ...
            }, 10 * headBobCount, loseHeadAnimation);
    });
    }else {
        headBobCount = 0;
        return
